...
function submit_status($d) {
	...
	If($d['rollno']!=0)
	$rst = mysql_query($stu_qry,$db) or die("DB Error Student".$stu_qry);
...
function register_user($p) {
	...
	$rs=mysql_query($qry,$db) or die(mysql_error()."Database error 1 - $qry");
	$lqry.=$qry."\n\r";
...
function submit_report($d) {
	...
	// echo $rep_qry;
	 mysql_query ($rep_qry,$db) or die ("DB Error update reporting!!!".mysql_error());
...
function display_form_data($db,$gssid,$prm) {
	...
	$retval.="<tr><td><b><a href=\"index.php?pid=1000&id=$gssid&ty=e&stype=$sc_type\">Edit</a></td></tr>";
	  $ifphoto = file_exists($ppath.$gssid.".jpg") ? "<tr><td colspan=\"2\">Photo:<br /><img src=\"sholarshipphotos/$gssid.jpg\"></td></tr>" : "<tr><td colspan=\"2\"><b>no photo submitted</b></td></tr>";
	$retval.=$ifphoto;
